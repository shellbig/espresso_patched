...
void set_particle_sat_mag(int part, double phi0);
...
// get_particle_axis_quat_body(const particle * p) Vector3d
// get_particle_axis(const particle * p)

inline Utils::Quaternion<double>
get_particle_axis_quat_body(Particle const *p) {
  return p->egg_model_params().axis_quat_body_fixed;
}

inline void get_particle_egg_model_params(Particle const *p, int &use_egg_model,
                                          double &egg_gamma,
                                          double &aniso_energy) {
